...
public class QuanLiDuLieu_GUI extends JFrame {
    ...
    public QuanLiDuLieu_GUI() {
        ...
        JLabel lblIcon = new JLabel(new ImageIcon(iconkm.getImage().getScaledInstance(
                (int) (iconkm.getIconWidth() * 0.15), (int) (iconkm.getIconHeight() * 0.15), Image.SCALE_SMOOTH)));
        // thu nhỏ ảnh
        ...
        btnSuaNV.addActionListener(evt -> {
            ...
            // (Giả sử bạn đã thêm hàm getThongTinSua trong NhanVien_DAO như tôi đã hướng
            // dẫn)
            Object[] data = null;
            try {
                NhanVien_DAO dao = new NhanVien_DAO();
                data = dao.getThongTinSua(maNV);
            } catch (Exception e) {
                e.printStackTrace();
            }
    ...
    private void refreshKMTable() {
        ...
        for (KhuyenMai k : khuyenMaiCtr.getDanhSachKhuyenMai()) {
            ...
            kmModel.addRow(
                    new Object[] { k.getMaKhuyenMai(), ten, phan, k.getNgayBatDauFormatted(),
                            k.getNgayKetThucFormatted(), k.getMoTa() });
        }
    }
